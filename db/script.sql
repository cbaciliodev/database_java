...
AS
BEGIN
SELECT T.ICOD_TEMA,T.VDESC_TEMA,T.CEST_TEMA
FROM DBO.TBL_TEMAS AS T
WHERE T.ICOD_TEMA LIKE @ICOD_TEMA;
END;
--EJECUTAR
EXEC SP_OBTEBER_TEMA_ID 1
...
END;
--EJECUTAR
EXEC SP_OBTEBER_LISTA_TEMA_ID 1
SELECT * FROM TBL_ACTIVIDADES


/*FUNCION*/
GO
CREATE FUNCTION F_OBTEBER_LISTA_TEMA_ID(@ICOD_ACTIVIDAD INT)
RETURNS TABLE
AS
RETURN(SELECT T.ICOD_TEMA,T.VDESC_TEMA,T.CEST_TEMA
FROM DBO.TBL_TEMAS AS T 
INNER JOIN DBO.TBL_ACTIVIDADES AS A
ON T.FK_ICOD_ACTIVIDAD = A.ICOD_ACTIVIDAD
WHERE T.FK_ICOD_ACTIVIDAD = A.ICOD_ACTIVIDAD
AND A.ICOD_ACTIVIDAD LIKE @ICOD_ACTIVIDAD)
GO
SELECT * FROM DBO.F_OBTEBER_LISTA_TEMA_ID(1);
